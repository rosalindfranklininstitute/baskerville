...
with open(f'logs/{JOB_ID}-{JOB_NAME}-{WORLD_RANK:04d}.env', 'w') as fp:
    subprocess.Popen(['env'], stdout=fp, stderr=fp).wait()
    fp.flush()
with open(f'logs/{JOB_ID}-{JOB_NAME}-{WORLD_RANK:04d}.nvsmi', 'w') as fp:
...
with open(f'logs/{JOB_ID}-{JOB_NAME}-{WORLD_RANK:04d}.nvsmi.csv', 'w') as fp:
